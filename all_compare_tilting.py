...
def double_evaluation_density_ratio(SASR, policy0, policy1, density_ratio, gamma, q_table, pi):
    ...
    for sasr in SASR:
        ...
        for state, action, next_state, reward in sasr:
            policy_ratio = policy1[state, action]/policy0[state, action]
            total_reward += density_ratio[state] * policy_ratio * (reward+gamma*np.sum(pi[next_state]*q_table[next_state,:]) -np.sum(pi[state]*q_table[state,:]) ) #### * discounted_t
            self_normalizer += 1.0###density_ratio[state] * policy_ratio ######* discounted_t
            #######discounted_t = gamma
    return total_reward / self_normalizer
...
def run_experiment(n_state, n_action, SASR, pi0, pi1, gamma):
    ...
    estimate_target =estimate_behavior*all_constants
    deno = np.sum(estimate_target,1)
    deno = deno[:,np.newaxis]
    estimate_target = estimate_target/deno
    pi1 = estimate_target
    ...
    est_model_double = np.sum(np.sum(Q_table*pi1,1).reshape(-1)*ddd)*(1-gamma)+double_evaluation_density_ratio(SASR, estimate_behavior, pi1, w, gamma, Q_table,pi1)
    est_model_double2 = np.sum(np.sum(Q_table*pi1,1).reshape(-1)*ddd)*(1-gamma)+double_evaluation_density_ratio2(SASR, estimate_behavior, pi1, w, gamma, Q_table,pi1)
    ...
    est_model_double_mis_q = np.sum(np.sum(Q_table_mis*pi1,1).reshape(-1)*ddd)*(1-gamma)+double_evaluation_density_ratio(SASR, estimate_behavior, pi1, w, gamma, Q_table_mis,pi1)
    est_model_double_mis_q2 = np.sum(np.sum(Q_table_mis*pi1,1).reshape(-1)*ddd)*(1-gamma)+double_evaluation_density_ratio2(SASR, estimate_behavior, pi1, w, gamma, Q_table_mis,pi1)
    ##### ratio misspecified 
    ...
    est_model_double_mis_w = np.sum(np.sum(Q_table*pi1,1).reshape(-1)*ddd)*(1-gamma)+double_evaluation_density_ratio(SASR, estimate_behavior, pi1, w_mis, gamma, Q_table,pi1)
    est_model_double_mis_w2 = np.sum(np.sum(Q_table*pi1,1).reshape(-1)*ddd)*(1-gamma)+double_evaluation_density_ratio2(SASR, estimate_behavior, pi1, w_mis, gamma, Q_table,pi1)
...
for i in range(2000):
    all_constants[i,:]=np.array([1.0,1.0,2.0,2.0,3.0,3.0])
pi_target =pi_behavior*all_constants
deno = np.sum(pi_target,1)
deno = deno[:,np.newaxis]
pi_target = pi_target/deno
...
for k in range(20):
    ...
    for i in range(9):
        ...
        print('----------------------')
        sys.stdout.flush()
    np.save('result/natural_ver={}nt={}ts={}gm={}.npy'.format(ver,nt,ts,gm), res)
